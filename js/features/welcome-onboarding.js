...
const questionE1 = $("[data-question]")
const optionsE1 = $("[data-options]")
const continueBtn = $("[data-continue]")
const backBtn = $("[data-back]")
const progressBar = $("[data-progress-bar]")

// Renderizza lo stati dello step
function render() {
    const state = onboarding.getState()
    const step = state.currentStep

    // aggiorna il testo della domanda
    questionE1.textContent = step.question

    // reset container option e applica layout dinamico
    optionsE1.innerHTML = ""
    optionsE1.className = `survey-options survey-options--${step.layout}`

    // genera dinamicamente tutte le opzioni
    step.options.forEach(function (option) {
        const optionElemet = createOptionElement(
            step, option, state.selectedOption
        )
        optionsE1.appendChild(optionElemet)
    });
